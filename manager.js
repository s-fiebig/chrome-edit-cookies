...
function resetTable() {
  ...
  var tableIndividualCookies = select("#individual_cookies");
    while (tableIndividualCookies.rows.length > 1) {
    tableIndividualCookies.deleteRow(tableIndividualCookies.rows.length - 1);
  }
}
...
function reloadCookieTable() {
  ...
  domains.forEach(function(domain) {
    ...
    cookies.forEach(function(cookie) {
      var rowCookies = tableIndividualCookies.insertRow(-1);
      rowCookies.insertCell(-1).innerText = domain;
      rowCookies.insertCell(-1).innerText = cookie.name;

      var inputField = document.createElement("input");
      inputField.value = cookie.value;
      var inputCell = rowCookies.insertCell(-1);
      inputCell.appendChild(inputField);
      inputCell.setAttribute("class", "input");

      var button = document.createElement("button");
      button.innerText = "edit";
      button.onclick = (function(oldCookie, inputField){
        return function() {
          editCookie(oldCookie, inputField);
        };
      }(cookie, inputField)); // using inputField to get inputField.value
      var buttonCell = rowCookies.insertCell(-1);
...
function editCookie(oldCookie, inputField) {
  oldCookie.value = inputField.value;
  var setCookie = createSetCookie(oldCookie)
  try{
    chrome.cookies.set(setCookie);
  } catch (e){
    //alert(e);
  }
  //reloadCookieTable();
...
function createSetCookie(cookie) {
  var setCookie = {};
    if(!cookie.hostOnly) {
    ...
    setCookie.httpOnly = cookie.httpOnly;
    setCookie.name = cookie.name;
    setCookie.path = cookie.path;
    setCookie.secure = cookie.secure;
    setCookie.storeId = cookie.storeId;
    setCookie.url = "http" + ((cookie.secure)? "s" : "") + "://" + cookie.domain + cookie.path;
